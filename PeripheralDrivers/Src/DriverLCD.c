...
void CMD_toLCD(I2C_Handler_t *ptrHandlerI2C, char cmd){
	...
	_L=((cmd<<4) & 0xf0);
	_T[0] = _U|0x0C;
	EscribirLCD(ptrHandlerI2C, _T[0]);
	_T[1] = _U|0x08;
	EscribirLCD(ptrHandlerI2C, _T[1]);
	_T[2] = _L|0x0C;
	EscribirLCD(ptrHandlerI2C, _T[2]);
	_T[3] = _L|0x08;
	EscribirLCD(ptrHandlerI2C, _T[3]);
}
...
void ResetScreenLCD(I2C_Handler_t *ptrHandlerI2C){
	char DataClean[64] = "                    ";
	for(int i=0;i<4;i++){
		moverCursorLCD(ptrHandlerI2C, i, 0);
		sendMSJCD(ptrHandlerI2C,DataClean);
	}
}
